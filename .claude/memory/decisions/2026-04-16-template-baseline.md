...
date: 2026-04-16
type: decision
tags: [#template, #baseline, #imported, #decision, #design, #state, #routing, #testing, #deploy]
scope: template
status: active
---

# Template Baseline — Architectural Decisions (imported)

Full import of the legacy `.claude/decisions.md` as of 2026-04-16. Kept as a single baseline snapshot rather than re-ADR'd individually (would add noise without new information). Future decisions = new files in this folder.
...
Going forward, each new architectural decision gets its own file: `YYYY-MM-DD-<slug>.md` with a focused scope and specific tags (not this omnibus set).
